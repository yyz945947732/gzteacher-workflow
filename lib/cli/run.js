...
import { debug as debugLog, error, warn } from '../utils/log.js';
...
async function runTasks(options) {
  ...
  try {
    ...
    if (isDebugOpen) {
      ...
      spinner.succeed(chalk.green('调试完毕'));
      return;
    }
    ...
    spinner.succeed(chalk.green(`tag 上传成功: ${context.tagName}`));
  } catch (err) {
